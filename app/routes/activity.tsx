...
export default function Activity({ loaderData }: Route.ComponentProps) {
  return <section className="card"><h1 className="text-xl font-semibold">Email activity</h1>
    ...
    {!loaderData.reports.length ? <p className="mt-6 text-sm">No email activity yet.</p> :
      <ul className="mt-4 divide-y divide-gray-200 dark:divide-gray-800">{loaderData.reports.map((report) =>
        <li key={report.id} className="py-3"><Link className="font-medium text-indigo-600" to={`/reports/${report.id}`}>{report.listName}{report.dryRun ? " · Dry run" : ""}</Link>
